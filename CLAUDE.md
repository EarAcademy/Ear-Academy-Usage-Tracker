...
1. `update_sales_dashboard.py` — **must run first** because it writes `paying_schools.json` (the AC roster).
2. `update_dashboard.py` — reads the fresh roster, then rebuilds `index.html`.
3. `update_velocity.py` — independent of the roster.

If sales fails (e.g., AC API outage), the usage script falls back to legacy mode (snapshot-only `paying()` filter with the `EXCLUDED_SCHOOLS` list) and prints a warning. Dashboards still update — just without the AC-driven school count refresh.
...
The canonical AC roster of paying schools — generated by `update_sales_dashboard.py`,
consumed by `update_dashboard.py`. **This file is the source of truth for which schools
are "paying" in the usage dashboard.**

```json
{
  "generated_at":    ISO-8601 timestamp,
  "generated_label": "13 May 2026 at 14:45",
  "filter": {
    "pipeline":  "6",                              // Customer Account Management
    "stages":    ["50 (Onboarding)", "51 (Activated)"],
    "currency":  "zar",
    "exclude":   "B2C accounts (title or account name contains 'B2C')"
  },
  "count":   int,                                  // total schools (currently 49)
  "schools": [
    {
      "deal_id":      str,                         // AC deal ID
      "title":        str,                         // AC deal title (canonical name)
      "stage":        "Activated" | "Onboarding",
      "stage_id":     "50" | "51",
      "value_zar":    int,                         // annual ZAR value
      "account_id":   str | null,                  // AC account ID
      "account_name": str | null                   // AC account display name
    }
  ]
}
```

Matching strategy used by `update_dashboard.py`:
1. Index every roster entry by its lowercased `title` AND `account_name`.
2. Apply explicit aliases from `SCHOOL_NAME_ALIASES` (snapshot name → matching key).
3. For each snapshot row, look up `school_name.lower()` in the index. If found → counted; if not → dropped.
...
### Add a paying school — or remove one
**Do nothing in this repo.** The roster is driven by AC: move the deal into Pipeline 6, stage Onboarding (50) or Activated (51), currency ZAR. The next `update_all_dashboards.sh` run picks it up automatically. Same for removing — move the deal out of Pipeline 6 or change its stage.

### Fix a snapshot ↔ AC name mismatch (school missing or duplicated on the dashboard)
A school is in `paying_schools.json` but not appearing on the Usage dashboard? Or appearing twice under slightly different names? That's a name-mismatch between the daily snapshot and AC.

1. Open `update_dashboard.py` and find the `SCHOOL_NAME_ALIASES` dict near the top.
2. Add a line: `"<exact snapshot name>": "<exact AC title or account_name>",`
3. Save. Next run picks it up.

### Tune Usage Patterns classification thresholds
Open `update_dashboard.py` and find the `PATTERN_RULES` dict near the Usage Patterns section. Adjust the constants (`power_min_tl`, `consistent_min_ratio`, etc.). Save and re-run — every school is re-classified.

### Add a school to the legacy exclusion list (fallback only)
Edit `EXCLUDED_SCHOOLS` in `update_dashboard.py` (near the top). This list **is only used when `paying_schools.json` is missing** — under normal operation, the AC roster does the gating. You almost certainly don't need to touch this.
...
| Usage dashboard school count differs from AC | `paying_schools.json` is stale or missing | Re-run `python3 update_sales_dashboard.py --no-push`, inspect the JSON, then run `python3 update_dashboard.py` |
| A real paying school is missing from the dashboard | Snapshot name doesn't match any roster `title` / `account_name` | Add an entry to `SCHOOL_NAME_ALIASES` in `update_dashboard.py` |
| `⚠️ No AC roster at paying_schools.json` | Sales script hasn't run yet (or AC API failed) | Run `python3 update_sales_dashboard.py` first, then the usage script |
| "Not Yet Active" pill shows wrong count | A school is in AC but hasn't logged in yet (expected behaviour) | None — this is the system working correctly. Check the school's onboarding status in AC. |
| Usage Patterns counts shift unexpectedly between runs | New snapshot data crossed a classification threshold | Tune `PATTERN_RULES` in `update_dashboard.py` if the new bucket assignment seems wrong |
...
- **AC Roster** — The canonical list of paying schools defined in `paying_schools.json`, fetched from AC Pipeline 6 (Customer Account Management), stages Onboarding + Activated, ZAR-only, B2C excluded.
- **ARR** — Annual Recurring Revenue
- **Name alias** — An explicit mapping in `SCHOOL_NAME_ALIASES` that connects a snapshot's school name to its corresponding AC roster entry, used when the strings don't match automatically (e.g. `dr.vanderross` → `Dr. V.D.Ross - C5`).
- **Not Yet Active** — A pattern bucket for schools that are paying customers in AC but have zero snapshot login activity. They appear on the dashboard with a flagged tile so onboarding follow-up is visible.
...
- **Power User / Hi-freq / Consistent / Bi-weekly / Early stage / One-time / Gone quiet** — Usage Patterns engagement classifications used in index.html (see Section 6 for the rules)
- **Time-in-stage bucket** — Number of days a deal has been in its current pipeline stage (u30 = under 30, etc.)
- **Pilot / Demo school** — Non-paying schools excluded from primary metrics (UK pilots feed the UK Pilot tile)
- **PATTERNS_BLOCK_START / PATTERNS_DATA_START** — Marker comments in `index.html` that bracket the auto-regenerated regions for the Usage Patterns tab. Never edit content between these by hand.
...
- [ ] **Roster check:** open `paying_schools.json`. Confirm `count` matches the number of deals in AC Pipeline 6, stages 50+51, ZAR, B2C excluded. The schools on the Usage dashboard's "All Paying Schools — Lifetime Activity" section should equal this number exactly.
- [ ] **Alias map sanity:** every entry in `SCHOOL_NAME_ALIASES` should resolve to a real roster `title` or `account_name`. If a school no longer exists in AC, its alias entry becomes dead weight — fine to leave but tidier to remove.
...
| Date | Change |
|---|---|
| 2026-05-13 | **AC roster becomes single source of truth.** New `paying_schools.json` is generated by `update_sales_dashboard.py` (Pipeline 6, stages 50+51, ZAR, B2C excluded). `update_dashboard.py` now uses it as the universe of paying schools — every snapshot row is gated against the roster. Removed obsolete `'C5'` exclusion from `EXCLUDED_SCHOOLS`. New `SCHOOL_NAME_ALIASES` dict reconciles snapshot ↔ AC name mismatches in one place. Added `--no-push` flag to the sales script for safe testing. |
| 2026-05-13 | **Usage Patterns tab now auto-generated.** Was previously a hand-maintained static block. New `calc_usage_patterns()` + `build_usage_patterns_html()` + `build_usage_patterns_js()` in `update_dashboard.py` rebuild both the visible HTML and the JS heatmap data on every run, using `PATTERNS_BLOCK_START/END` and `PATTERNS_DATA_START/END` marker comments. New **Not Yet Active** pattern bucket and summary tile surface paying schools that haven't logged in yet. Heatmap CSS switched to `grid-template-columns: minmax(220px,1fr) auto 110px` so the Pattern badge stays aligned regardless of how many weeks of data exist. Order swapped in `update_all_dashboards.sh` — sales runs before usage so the roster is fresh when read. |
